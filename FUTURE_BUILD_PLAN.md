...
outliers. v3 correctly made taxonomy and storage-key conflict handling
visible, but the later metric-level review found that the generic conflict
rule is too blunt for revenue. `Revenues` and an ASC 606 revenue concept can
legitimately coexist with different economics in one filing; withholding both
removed current revenue for 43 members. The structural migration remains
auditable and replayable, but its revenue policy is now a known research-data
defect, not a correctness result to defend wholesale.
...
| Unattended paper cycle (autopilot) | **Built 2026-08-13; fills remain manual; current trial drifted pending prospective restart** | The trial has never recorded a fill; proposals expired unexecuted and the frozen policy now intentionally blocks the predecessor | `src/aios/autopilot.py` + `aios autopilot`, wired as an `ExecStartPost` on `aios-us-daily.service`. The scheduled unit carries no `--confirm-simulated`, so it may stage and report but cannot approve a fill. The command implements no governed logic: readiness, prospective timing, registered-proposal, checksum and frozen-policy gates still run inside `paper.py`/`forward.py`, and it reapplies the trial-readiness, `top_n`-match and unresolved-proposal guards. A refused fill uses a leading `-` so it cannot turn a deliberate paper refusal into a data-refresh failure |
| Company Facts revenue policy v4 | **Built; prior 500/500 review invalidated, activation blocked pending fresh evidence** | 43 of the 61 stale-revenue members were a v3 precedence artifact; broad tag addition under v3 also broke 16 previously correct names | Immutable `sec-companyfacts-v4` now limits declared concept precedence to revenue, keeps non-revenue conflicts fail-closed, scopes assessed-tax revenue to 16 reviewed issuers, gives four utilities their complete operating-revenue concept, derives VLO revenue net of excise, scopes EQR lease revenue, and suppresses incomparable financial/component-only cases. The plan/prepare/activate path reuses exact v2 payloads, accepted v3 receipts, row lineage, backup, CAS, disposable restore, tombstones and append-only receipts, and pins the exact v3-to-v4 transition. Read-only rechecks on 2026-08-15 found 471/500 eligible at the old 2026-08-13 boundary (29 `current_relation_mismatch`; hash `bba26b91c87455de5b222834ea025813d581ece7a545fe3b7026023e58e24c0d`) and 0/500 at the 2026-08-15 boundary (471 `failed_ingest`, 29 `ambiguous_identity`; hash `e4b936b8cc7cc9b89233743c2313d60ccfc44eaebb24e18e58d4b6545204a769`). No plan publication, activation, score change, paper action or rollover occurred; fresh clean source/reference evidence and a new 500/500 review are required |
| `share_count_jump` filing-vintage detector v2 | **Built and regression-tested 2026-08-15; ledger cases still require a fresh recorded scan** | Period ordering manufactured phantom jumps across filing vintages | Pass A compares one latest period per filing date in filing-vintage order; pass B detects contradictions within a single filing vintage. It never normalizes with `prices.split_ratio`, because provider price-continuity factors are not legal share multipliers. The measured 41 findings/25 issuers became 35/33 and HON's three phantom comparisons collapse to the single real ~-50.6% event |
| HON price/legal-share action contract | **Parser and reviewed policy built; historical activation blocked** | Yahoo's 0.9535 factor does not encode the legal 1-for-2 reverse split, and HONA distribution value is not represented | Immutable `yfinance-normalized-v4` keeps raw close on a contemporaneous basis, preserves Yahoo's factor separately, and only projects an exact identity/date/factor-reviewed legal share ratio. HON 2026-06-29 records legal ratio 0.5 but remains `actions_complete = false` because the HONA distribution is unsupported. Existing v3 rows were not rewritten; retained raw snapshots begin after the event dates, so migration cannot claim replay proof it does not have |
| Price-anomaly correction clearance | **Built and regression-tested 2026-08-15** | Absence from a later scan alone did not prove that a price was corrected from retained provider evidence | `source_corrected`/`mapping_corrected` for `price_action_mismatch@1.0.0` now requires the exact previous/current rows to replay from checksum-verified provider snapshots, match stored economics, postdate the finding and no longer trigger. Direct DuckDB edits cannot create the proof |
| Historical data cases hard-block prospective simulation | **Open — design question, do not silently weaken** | This is the actual reason the paper trial never progresses | `operator_preflight`'s operations capability is `available` only at **zero** unresolved anomaly cases and incidents. Anomaly cases are findings about *historical* data (Citigroup's 2009 share count, CRWD's 2019 zero-share filings), yet they gate `forward-rollover`, which is the only path that closes an expired proposal, which in turn blocks creating the next one. Cases accumulate faster than they are dispositioned, so the trial deadlocks permanently. A 2009 share-count finding has no bearing on whether a 2026 prospective simulation is safe. Either the gate should distinguish current-decision-date findings from historical backlog, or the backlog must be worked down and kept at zero — but weakening the gate to unblock automation would be exactly the wrong fix and is deliberately not done here |
| Paper account identity collision | **Fixed for newly initialized accounts; existing trial restart pending** | A shared hardcoded ID made the cross-account proposal guard ineffective | `paper-init` now generates a unique `paper-account-<uuid>` identity and tests prove two accounts cannot collide. Existing account/proposal artifacts are deliberately not rewritten; the fix changes the frozen bundle and therefore takes effect through a planned prospective restart, not an in-place hotfix |
...
executed, and the fingerprint absent. Fundamental-coverage clearance
additionally binds to a source-provenanced accepted SEC ingest with positive
verified rows. Price-action clearance binds the exact comparison pair to
checksum-verified retained provider snapshots and rechecks its economics.
Deferral needs a future review time and remains unresolved. Case reads and
mutations replay the immutable ordered lifecycle and verify the mutable
projection. No disposition authorizes a data edit or bypasses a readiness gate.
This local integrity check trusts the released AIOS code and OS/filesystem
access controls; it is not cryptographic attestation against an actor able to
replace both code and evidence.

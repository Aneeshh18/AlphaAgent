...
def _rows_to_df(rows) -> pd.DataFrame:
    ...
    for r in rows:
        data.append({
            ...
            "Gross Margin %": round(r.gross_margin * 100, 1) if r.gross_margin is not None else None,
            ...
            "Market Cap ($B)": round(r.market_cap / 1e9, 1) if r.market_cap is not None else None,
        })
...
if view == "🌍 Universe Ranking":
    ...
    st.caption(f"As of {as_of} • {len(df)} tickers • Quality 60% / Value 40%")
    ...
    plot_df["Market Cap ($B)"] = plot_df["Market Cap ($B)"].fillna(median_mcap if pd.notna(median_mcap) else 100)
    fig = px.scatter(
        plot_df, x="Value", y="Quality", text="Ticker",
        size="Market Cap ($B)", size_max=45,
        color="QV Score", color_continuous_scale="RdYlGn",
    ...
    with col1:
        ...
        st.dataframe(df.head(5)[["Ticker", "Grade", "QV Score", "Quality", "Value"]],
                     hide_index=True, width="stretch")
    with col2:
        st.markdown("##### 🔴 Bottom 5 by QV")
        st.dataframe(df.tail(5)[["Ticker", "Grade", "QV Score", "Quality", "Value"]],
                     hide_index=True, width="stretch")
...
# ----------------------------------------------------------------------
    ...
    # Header card
    grade_color = {"A+": "green", "A": "green", "B": "blue", "C": "orange", "D": "red"}.get(row["Grade"], "gray")
    ...
    col4.metric("Market Cap", f"${row['Market Cap ($B)']}B" if pd.notna(row['Market Cap ($B)']) else "N/A")
    ...
    qc3.metric("Gross Margin", f"{row['Gross Margin %']}%" if pd.notna(row['Gross Margin %']) else "N/A")
    qc4.metric("Piotroski F", f"{row['Piotroski F']}/4" if pd.notna(row['Piotroski F']) else "N/A")

    # Value breakdown
    st.subheader("🟩 Value breakdown")
    st.caption("Lower multiples = cheaper. Score reflects percentile rank vs universe (higher = cheaper).")
    ...
    if snap:
        ...
        # Quality components
        for name, val in [("ROIC", snap.roic), ("FCF mgn", snap.fcf_margin),
                          ("Gross mgn", snap.gross_margin)]:
            if val is not None:
                peer_rows = compute_composite(sorted(df["Ticker"].tolist()), as_of, store)
                peers = []
                for pr in peer_rows:
                    v = {"ROIC": pr.roic, "FCF mgn": pr.fcf_margin, "Gross mgn": pr.gross_margin}.get(name)
                    if v is not None: peers.append(v)
                pct = fc.percentile_rank(val, peers)
                if pct is not None:
                    radar_labels.append(name); radar_values.append(pct * 100)
        ...
        for name, val in [("Cheap P/E", snap.pe), ("Cheap EV/E", snap.ev_ebitda)]:
            if val is not None and val > 0:
                ...
                for pr in peer_rows:
                    v = getattr(pr, attr)
                    if v is not None and v > 0: peers.append(v)
...
### Composite QV
Weighted blend: `QV = 0.6 × Quality + 0.4 × Value` (weights become regime-adjusted
in the next phase when the macro overlay is wired in).
...
- **Piotroski F-Score** uses 4 of 9 criteria in MVP (leverage/efficiency criteria
  need prior-year balance-sheet snapshots — coming next).
...
""")
    st.info("Full architecture decisions: see `ARCHITECTURE.md`. Strategy basis: see the original prompt analysis.")
